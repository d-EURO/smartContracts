...
const deploy: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (["hardhat", "localhost", "sepolia"].includes(hre.network.name)) {
    console.log("\nSetting MockVEUR Token Bridge");
    try {
      const mockTokenDeployment = await get("TestToken");
      veurAddress = mockTokenDeployment.address;
    } catch (err: unknown) {
      veurAddress = await getAddress();
      if (veurAddress == "") {
        throw err;
      }
    }

    applicationMsg = "MockVEUR Token Bridge";
  } else {
    console.log("\nDeploying VEUR Token Bridge");
    veurAddress = "0x6ba75d640bebfe5da1197bb5a2aff3327789b5d3";
    applicationMsg = "VEUR Bridge";
  }
  const dEURODeployment = await get("EuroCoin");

  let dLimit = floatToDec18(limit);
  console.log("Deploying VEUR StablecoinBridge with other =", veurAddress, "limit = ", limit, "EUR", "and weeks =", weeks);
  ...
  if (!["hardhat", "localhost"].includes(hre.network.name)) {
    ...
    console.log(
      `Verify StablecoinBridge:\nnpx hardhat verify --network ${hre.network.name} ${bridgeAddr} ${veurAddress} ${dEURODeployment.address} ${dLimit}\n`
    );
  
    try {
      await run("verify:verify", {
        address: bridgeAddr,
        constructorArguments: [veurAddress, dEURODeployment.address, dLimit, weeks],
      });
