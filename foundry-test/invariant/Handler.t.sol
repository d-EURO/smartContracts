...
contract Handler is StatsCollector {
    ...
    /// @dev Save JUSD into Savings
    function saveJUSD(uint8 eoaIdx, uint192 amount, bool compound) public {
        if (!shouldExecute(20)) return;

        address actor = s_env.eoas(eoaIdx);
        amount = uint192(bound(amount, 1e18, 500_000e18));

        recordAction("saveJUSD");
        s_env.mintJUSD(actor, amount);
        Savings sav = s_env.savings();
        (uint192 preSaved,) = sav.savings(actor);

        vm.startPrank(actor);
        s_env.jusd().approve(address(sav), amount);
        try sav.save(amount, compound) {
            (uint192 postSaved,) = sav.savings(actor);
            assertGe(postSaved, preSaved + amount, "saveJUSD: saved did not increase correctly");
        } catch {
            recordRevert("saveJUSD");
        }
        vm.stopPrank();
    }

    /// @dev Withdraw JUSD from Savings
    function withdrawSavings(uint8 eoaIdx, uint192 amount) public {
        ...
        uint256 preBalance = s_env.jusd().balanceOf(actor);

        vm.startPrank(actor);
        try sav.withdraw(actor, amount) {
            uint256 postBalance = s_env.jusd().balanceOf(actor);
            assertGt(postBalance, preBalance, "withdrawSavings: JUSD balance did not increase");
        } catch {
    ...
    /// @dev Invest JUSD into Equity (buy nJPS)
    function investEquity(uint8 eoaIdx, uint256 amount) public {
        ...
        vm.startPrank(actor);
        s_env.jusd().approve(address(eq), amount);
        try eq.invest(amount, 0) {
            uint256 postShares = eq.balanceOf(actor);
            assertGt(postShares, preShares, "investEquity: nJPS balance did not increase");
        } catch {
    ...
    /// @dev Redeem nJPS for JUSD
    function redeemEquity(uint8 eoaIdx, uint256 shares) public {
        if (!shouldExecute(5)) return;

        address actor = s_env.eoas(eoaIdx);
        Equity eq = s_env.equity();
        if (block.number <= eq.lastInboundBlock(actor)) return;
        ...
        try eq.redeem(actor, shares) {
            uint256 postBalance = s_env.jusd().balanceOf(actor);
            assertGt(postBalance, preBalance, "redeemEquity: JUSD balance did not increase");
        } catch {
    ...
    /// @dev Mint JUSD via StablecoinBridge (deposit EUR)
    function bridgeMint(uint8 eoaIdx, uint256 amount) public {
        ...
        uint256 preJusd = s_env.jusd().balanceOf(actor);
        uint256 preMinted = br.minted();

        vm.startPrank(actor);
        s_env.eurToken().approve(address(br), amount);
        try br.mint(amount) {
            uint256 postJusd = s_env.jusd().balanceOf(actor);
            uint256 postMinted = br.minted();
            assertEq(postJusd, preJusd + amount, "bridgeMint: incorrect JUSD balance");
    ...
    /// @dev Burn JUSD via StablecoinBridge (withdraw EUR)
    function bridgeBurn(uint8 eoaIdx, uint256 amount) public {
        ...
        vm.startPrank(actor);
        s_env.jusd().approve(address(br), amount);
        try br.burn(amount) {
    ...
    function clonePosition(uint8 eoaIdx, uint8 parentIdx, uint256 collateral, uint256 mintAmount) public {
        ...
        s_env.mintJUSD(actor, openingFee);
        ...
        vm.startPrank(actor);
        s_env.jusd().approve(address(s_env.mintingHub()), openingFee);
    ...
    function snapshot(Position position) internal view returns (Snapshot memory) {
        ...
        return
            Snapshot({
                ...
                // Position owner
                owner: owner,
                ownerBalanceJUSD: s_env.jusd().balanceOf(owner),
