...
contract Invariants is TestHelper {
    ...
    function setUp() public {
        ...
        /// Savings
        selectors[11] = Handler.saveJUSD.selector;
    ...
    function invariant_totalSupplyConsistency() public view {
        ...
        totalBalances += jusd.balanceOf(address(s_env.bridge()));
        totalBalances += jusd.balanceOf(address(s_env.mintingHub()));
        for (uint256 i = 0; i < 5; i++) totalBalances += jusd.balanceOf(s_env.eoas(i));

        // Include all positions (originals + clones)
        Position[] memory positions = s_env.getPositions();
        for (uint256 i = 0; i < positions.length; i++) {
            totalBalances += jusd.balanceOf(address(positions[i]));
        }
    ...
    function invariant_reserveSolvency() public view {
        JuiceDollar jusd = s_env.jusd();
        uint256 reserveBalance = jusd.balanceOf(address(jusd.reserve()));
        uint256 minterReserve = jusd.minterReserve();
    ...
    /// @dev MISS-5: PositionRoller holds zero JUSD between transactions (flash loan net-to-zero)
    function invariant_rollerNetZero() public view {
        uint256 rollerBalance = s_env.jusd().balanceOf(address(s_env.positionRoller()));
        assertEq(rollerBalance, 0, "MISS-5: PositionRoller holds JUSD (flash loan not repaid)");
    }

    /// @dev MISS-7: nJPS total supply cannot exceed uint96 max
    function invariant_ndepsSupplyCap() public view {
        Equity equity = Equity(address(s_env.jusd().reserve()));
        assertLe(equity.totalSupply(), type(uint96).max, "MISS-7: nJPS supply exceeds uint96 max");
    }

    /// @dev Savings solvency: contract holds enough JUSD to cover all saved + claimable interest
    function invariant_savingsSolvency() public view {
        Savings sav = s_env.savings();
        JuiceDollar jusd = s_env.jusd();
        uint256 savingsBalance = jusd.balanceOf(address(sav));
    ...
    /// @dev Bridge backing: EUR token balance must cover outstanding minted JUSD
