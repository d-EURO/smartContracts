...
describe('CoinLendingGateway Tests', () => {
  ...
  before(async () => {
    ...
    // Create mockXEUR and bridge to bootstrap JUSD
    ...
    bridge = await BridgeFactory.deploy(mockXEUR.getAddress(), JUSD.getAddress(), bridgeLimit, 30);

    // Initialize JUSD
    await JUSD.initialize(bridge.getAddress(), 'XEUR Bridge');
