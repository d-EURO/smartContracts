...
import { floatToDec18, dec18ToFloat, DECIMALS } from '../../scripts/utils/math';
...
describe('Position Tests', () => {
  ...
  before(async () => {
    ...
    bridge = await bridgeFactory.deploy(await mockXUSD.getAddress(), await JUSD.getAddress(), limit, weeks);
    ...
    await mockXUSD.connect(owner).approve(await bridge.getAddress(), limit / 3n);
  ...
  describe('Use Minting Hub', () => {
    ...
    it('should revert minting from non owner', async () => {
      await expect(positionContract.connect(alice).mint(owner.address, 100)).to.be.revertedWithCustomError(
        positionContract,
    ...
    it('should revert minting when there is a challange', async () => {
      ...
      await mintingHub.challenge(await positionContract.getAddress(), fchallengeAmount, price);
    ...
    it('should revert cloning for invalid position', async () => {
      ...
      await expect(
        mintingHub
          .connect(alice)
          .clone(alice.address, owner.address, fInitialCollateralClone, fMintAmount, newExpiration, 0),
    ...
    it('should revert cloning when new expiration is greater than original one', async () => {
      ...
      await expect(
        mintingHub
          .connect(alice)
          .clone(alice.address, positionAddr, fInitialCollateralClone, fMintAmount, expiration + 100n, 0),
      ).to.be.revertedWithCustomError(positionContract, 'InvalidExpiration');
    });
    it('should revert initializing again', async () => {
      await expect(positionContract.initialize(positionAddr, 0)).to.be.revertedWithCustomError(
        positionContract,
        'NotHub',
      );
    });
    ...
    it('clone position', async () => {
      ...
      let tx = await mintingHub
        .connect(alice)
        .clone(alice.address, positionAddr, fInitialCollateralClone, fMintAmount, newExpiration, 0);
      let rc = await tx.wait();
      const topic = '0xc9b570ab9d98bdf3e38a40fd71b20edafca42449f23ca51f0bdcbf40e8ffe175'; // PositionOpened
      const log = rc?.logs.find((x) => x.topics.indexOf(topic) >= 0);
    ...
    it('correct fees charged', async () => {
      ...
      let cloneFeeCharged = fBalanceAfter - fGlblZCHBalanceOfCloner - mintAfterFees * BigInt(1e18);
    ...
    it('clone position with too much mint', async () => {
      ...
      let pendingTx = mintingHub
        .connect(alice)
        .clone(alice.address, positionAddr, fInitialCollateralClone * 1000n, initialLimit, expiration, 0);
    ...
    it('should revert on price adjustments when expired', async () => {
      ...
      await expect(positionContract.adjustPrice(currentPrice / 2n)).to.be.revertedWithCustomError(
        positionContract,
        'Expired',
      );
    });
    it('should revert on price adjustments when expired', async () => {
      ...
      await positionContract.adjust(principal, collateralBalance, currentPrice, false); // don't revert if price is the same
      await expect(
        positionContract.adjust(principal, collateralBalance, currentPrice / 2n, false),
    ...
    it('should revert reducing limit from non hub', async () => {
      ...
      await expect(positionContract.notifyMint(0)).to.be.revertedWithCustomError(positionContract, 'NotHub');
      await expect(positionContract.notifyRepaid(0)).to.be.revertedWithCustomError(positionContract, 'NotHub');
    });
    ...
    it('should revert reducing limit when there is a challenge', async () => {
      ...
      challengeNumber++;
      await expect(positionContract.assertCloneable()).to.be.revertedWithCustomError(positionContract, 'Challenged');
    });
  });
  ...
  describe('challenge active', () => {
    ...
    it('should revert challenging with zero collateral', async () => {
      let price = await positionContract.price();
      await expect(mintingHub.challenge(positionAddr, 0, price)).to.be.revertedWithCustomError(
        positionContract,
        'ChallengeTooSmall',
      );
    });
    it('should revert challenging for invalid position address', async () => {
      ...
      let fchallengeAmount = floatToDec18(challengeAmount);
      let price = await positionContract.price();
      await expect(mintingHub.challenge(owner.address, fchallengeAmount, price)).to.be.revertedWithCustomError(
        mintingHub,
        'InvalidPos',
      );
    });
    it('should revert challenging with different position price', async () => {
      ...
      let fchallengeAmount = floatToDec18(challengeAmount);
      let price = await positionContract.price();
      await expect(mintingHub.challenge(positionAddr, fchallengeAmount, price + 100n)).to.be.revertedWithCustomError(
        mintingHub,
        'UnexpectedPrice',
      );
    });
    it('should revert challenging zero amount or less than minimal collateral', async () => {
      ...
      await expect(mintingHub.challenge(positionAddr, floatToDec18(0.1), price)).to.be.revertedWithCustomError(
        positionContract,
        'ChallengeTooSmall',
      );
    });
    it('bid on challenged, flat sale, not expired position', async () => {
      ...
      await mockVOL.approve(await mintingHub.getAddress(), fchallengeAmount);
      let tx = await mintingHub.challenge(positionAddr, fchallengeAmount, price);
      challengeNumber++;
    ...
    it('bid on challenge with amount larger than collateral balance of position', async () => {
      ...
      let liqPrice = await mintingHub.price(challengeNumber);
      expect(liqPrice).to.be.approximately(price, liqPrice / 100n);
      ...
      await JUSD.connect(bob).approve(await mintingHub.getAddress(), bidAmountJUSD + propInterest);
    ...
    it('bid on challenged, auction sale, not expired position', async () => {
      ...
      let tx = await mintingHub.connect(charles).challenge(positionAddr, fchallengeAmount, price);
      challengeNumber++;
      const challenge = await mintingHub.challenges(challengeNumber);
      const challengeData = await positionContract.challengeData();
  ...
  describe('challenge clone', () => {
    ...
    it('bid on challenged, expired position', async () => {
      ...
      let interest = (await cloneContract.getDebt()) - (await cloneContract.principal());
      await JUSD.connect(alice).approve(await mintingHub.getAddress(), bidAmountJUSD + interest);
  ...
  describe('adjusting price', async () => {
    ...
    it('should revert adjusting price when there is pending challenge', async () => {
      ...
      await mintingHub.challenge(positionAddr, fchallengeAmount, price);
      challengeNumber++;
      await expect(positionContract.adjustPrice(floatToDec18(1500))).to.be.revertedWithCustomError(
        positionContract,
        'Challenged',
      );
    });
    it('should increase cooldown for 3 days when submitted price is greater than the current price', async () => {
      ...
      await expect(positionContract.adjustPrice(initialPrice)).to.be.emit(positionContract, 'MintingUpdate');
    ...
    it('should revert adjusting to lower price when it lowers the collateral reserves below minted values', async () => {
      ...
      await expect(positionContract.adjustPrice(floatToDec18(100))).to.be.revertedWithCustomError(
        positionContract,
    ...
    it('should revert adjusting price when new price is greater than required to cover max principal', async () => {
      ...
      await expect(positionContract.adjustPrice(floatToDec18(6000))).to.be.revertedWithCustomError(
        positionContract,
        'PriceTooHigh',
      );
    });
    it('should revert adjusting price when new price is greater than double the current price', async () => {
      await evm_increaseTimeTo(await positionContract.cooldown());
      const price = await positionContract.price();
      await expect(positionContract.adjustPrice((price * 21n) / 10n)).to.be.revertedWithCustomError(
        positionContract,
        'PriceTooHigh',
      );
    });
    it('should allow price decrease during cooldown', async () => {
      ...
      await expect(positionContract.adjustPrice(higherPrice + floatToDec18(1))).to.be.revertedWithCustomError(
        positionContract,
        'Hot',
      );
  ...
  describe('adjusting price with reference', async () => {
    ...
    it('should work with adjust() using reference position parameter', async () => {
      ...
      // Use the 5-parameter adjust function
      await localPositionContract.adjustWithReference(principal, collBal, newPrice, referencePositionAddr, false);
    ...
    it('should trigger cooldown when using adjust() with address(0) as reference', async () => {
      ...
      await localPositionContract.adjustWithReference(principal, collBal, newPrice, ethers.ZeroAddress, false);
    ...
    it('should revert when reference position is expired', async () => {
      ...
      let tx = await mintingHub.openPosition(
        ...
        fInitialCollateral,
        initialLimit,
        14n * 24n * 3600n, // 14 day init period
        shortDuration,
        BigInt(3 * 86400),
        BigInt(fee * 1_000_000),
        floatToDec18(9000),
  ...
  describe('adjusting position', async () => {
    ...
    it('should revert adjusting position from non position owner', async () => {
      await expect(positionContract.connect(alice).adjust(0, 0, 0, false)).to.be.revertedWithCustomError(
        positionContract,
        'OwnableUnauthorizedAccount',
      );
    });
    ...
    it('owner can repay debt partially', async () => {
      ...
      const returnedReserve = await JUSD.calculateAssignedReserve(
        amountToRepay,
        await positionContract.reserveContribution(),
      );
  ...
  describe('rescuing tokens', () => {
    it('should revert rescuing tokens from non position owner', async () => {
      ...
      await expect(
        positionContract.connect(alice).rescueToken(await JUSD.getAddress(), owner.address, amount),
      ).to.be.revertedWithCustomError(positionContract, 'OwnableUnauthorizedAccount');
    });
    it('owner can rescue non-collateral ERC20 tokens locked on position contract', async () => {
      ...
      await positionContract.rescueToken(await JUSD.getAddress(), owner.address, amount);
    ...
    it('should revert when trying to rescue collateral', async () => {
      const amount = floatToDec18(1);
      await expect(
        positionContract.rescueToken(await mockVOL.getAddress(), owner.address, amount),
      ).to.be.revertedWithCustomError(positionContract, 'CannotRescueCollateral');
    });
  });
  describe('returning postponed collateral', async () => {
    it('should return pending postponed collaterals (Need to find more exact scenarios)', async () => {
      await mintingHub.returnPostponedCollateral(await mockVOL.getAddress(), owner.address);
    });
  });
  describe('notifying loss', async () => {
    ...
    beforeEach(async () => {
      ...
      const expiration = await positionContract.expiration();
      tx = await mintingHub.clone(alice.address, positionAddr, fInitialCollateral, cloneLimit, expiration, 0);
      rc = await tx.wait();
      log = rc?.logs.find((x) => x.topics.indexOf(topic) >= 0);
      clonePositionAddr = '0x' + log?.topics[2].substring(26);
      clonePositionContract = await ethers.getContractAt('Position', clonePositionAddr);
    ...
    it('should transfer loss amount from reserve to minting hub when notify loss', async () => {
      ...
      await JUSD.connect(bob).approve(await mintingHub.getAddress(), expCost);
      let tx = await mintingHub.connect(bob).bid(challengeNumber, fchallengeAmount, false);
      await expect(tx)
        .to.emit(mintingHub, 'ChallengeSucceeded')
        .withArgs(ethers.getAddress(clonePositionAddr), challengeNumber, expOffer, fchallengeAmount, fchallengeAmount)
        .emit(JUSD, 'Loss');
  ...
  describe('position expiration auction', () => {
    ...
    it('force sale should fail before expiration', async () => {
      await expect(test.forceBuy(pos.getAddress(), 1n)).to.be.revertedWithCustomError(pos, 'Alive');
    });
    ...
    it('force sale should succeed after expiration', async () => {
      ...
      await test.approveJUSD(pos.getAddress(), floatToDec18(10_000) + propInterest);
    ...
    it('force sale at liquidation price should succeed in cleaning up position', async () => {
      ...
      const reservePortion = await JUSD.calculateAssignedReserve(
        await pos.principal(),
    ...
    it('should revert due to dust protection', async () => {
      ...
      await expect(test.forceBuy(pos.getAddress(), 63n)).to.revertedWithCustomError(mintingHub, 'LeaveNoDust');
    });
  ...
  describe('Fix interest rate tests', () => {
    ...
    it('should fix rate on creation and not update if lead rate changes later (repay at old rate)', async () => {
      ...
      const expectedInterest = (usablePrincipal * expectedAnnualRate * (2n * timePassed)) / (1000000n * 365n * 86400n);
    ...
    it('should update to new lead rate when the loan is increased', async () => {
      ...
      const expectedInterestBeforeMint =
        (usablePrincipal * (initialLeadratePPM + riskPremium) * totalLoanTime) / (1000000n * 365n * 86400n);
      expect((await positionContract.getDebt()) - initialMintAmount).to.be.approximately(
      ...
      const expectedDebt =
        principalAfterMint +
        expectedInterestBeforeMint +
        (usableNewMint * (newLeadratePPM + riskPremium) * timeAtNewLeadrateAfterMint) / (1000000n * 365n * 86400n);
    ...
    it('should preserve old rate until rolling into a new position with extended expiry (new rate applies after roll)', async () => {
      ...
      const expectedInterestNewPos =
        (usableMintedInNewPos * newFixedRate * timeUnderNewRate) / (1000000n * 365n * 86400n);
  ...
  describe('Liquidation tests', () => {
    ...
    it('Balance checks before and after liquidation', async () => {
      ...
      await JUSD.connect(bob).approve(await mintingHub.getAddress(), interest + floatToDec18(1));
      let colBalanceBeforePos = await mockVOL.balanceOf(positionAddr);
      let volBalanceBeforeBob = await mockVOL.balanceOf(bob.address);
      let balanceBeforeBob = await JUSD.balanceOf(bob.address);
      let balanceBeforeEquity = await JUSD.balanceOf(await equity.getAddress());
      let balanceBeforeMintingHub = await JUSD.balanceOf(await mintingHub.getAddress());
      ...
      let balanceAfterMintingHub = await JUSD.balanceOf(await mintingHub.getAddress());
      let debtAfter = await positionContract.getDebt();

      let expPrincipalToPay = (initialMintAmount * bidSize) / colBalanceBeforePos;
      ...
      expect(balanceBeforeBob - balanceAfterBob).to.be.approximately(offer, floatToDec18(1));
      // expect(balanceAfterEquity).to.be.eq(balanceBeforeEquity);
      expect(debtBefore - debtAfter).to.be.approximately(expPrincipalToPay + expInterestToPay, floatToDec18(1));
    });
  });
  ...
  describe('Interest overcollateralization', () => {
    ...
    it('allows withdrawing collateral when interest is sufficiently overcollateralized', async () => {
      ...
      await expect(positionContract.withdrawCollateral(owner.address, withdrawTooMuch)).to.be.revertedWithCustomError(
        positionContract,
        'InsufficientCollateral',
      );
