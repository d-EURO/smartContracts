...
describe("ERC3009 Tests", () => {
  ...
  it("Should cancel the authorization", async () => {
    ...
    const sig = await getCancelAuthorizationSignature(
      testToken,
      owner,
      owner.address,
      nonce,
    );
    await testToken.cancelAuthorization(
      owner.address,
      nonce,
      sig.v,
      sig.r,
  ...
  describe("Testing transfer and before transfer", () => {
    it("Should transfer the token", async () => {
      ...
      await testToken.transferWithAuthorization(
        owner.address,
        alice.address,
        oneETH,
        0,
    ...
    it("Should fail the nonce is canceled", async () => {
      ...
      const sig2 = await getTransferAuthorizationSignature(
        testToken,
        owner,
        owner.address,
        alice.address,
      ...
      await expect(
        testToken.transferWithAuthorization(
          owner.address,
          alice.address,
          oneETH,
          0,
