...
contract Position is Ownable, IPosition, MathUtil {
     ...
     * the effective principal (principal minus reserve), the annual interest rate, and the elapsed time.
    ...
    function _calculateInterest() internal view returns (uint256 newInterest) {
        ...
        if (timestamp > lastAccrual && principal > 0) {
            ...
            // Interest is calculated only on the effective principal (what user actually received)
            uint256 effectivePrincipal = (principal * (1_000_000 - reserveContribution)) / 1_000_000;
            newInterest += (effectivePrincipal * fixedAnnualRatePPM * delta) / (365 days * 1_000_000);
        }
